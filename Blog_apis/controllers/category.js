...
const {getPagination,getPagingData,Checknegative,searchStatus} = require('../utils/transactions-response')
...
// search active and inactive categories 
const searchCategoryStatus = async (req, res) => {
    const statusUpdate = req.params.status
    try {
        const category = await searchStatus(statusUpdate);
        if (category) {
            res.json(category);
        } else {
            res.status(404).json({message:"Status of Category not found"})
        }
    } catch (error){
        console.log(error)
